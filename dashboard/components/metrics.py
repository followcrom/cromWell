...
def display_activity_metrics(dfs: Dict[str, pd.DataFrame]) -> None:
    """
...
def display_extended_activity_metrics(dfs: Dict[str, pd.DataFrame]) -> None:
    """
    ...
    with col1:
        if df_hrv is not None and not df_hrv.empty:
            hrv = int(df_hrv["dailyRmssd"].iloc[0])
    ...
    with col3:
        if df_activity_records is not None and not df_activity_records.empty:
            ...
            st.metric("Logged Activities", count)
        else:
...
def display_activity_summary_table(dfs: Dict[str, pd.DataFrame]) -> None:
    """
